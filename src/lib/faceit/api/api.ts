...
export async function fetchHubMembers(
  ...
  const response = await faceitClient.get<{ items: HubMember[] }>(
    `/hubs/${hubId}/members`,
    {
      params,
    },
...
export async function getAllHubMembers(hubId: string) {
  const loadedMembers: HubMember[] = [];
  let offset = 0;
  const limit = 50;
  let newMembers: HubMember[];

  do {
    newMembers = await fetchHubMembers(hubId, { offset, limit });
    loadedMembers.push(...newMembers);
    offset += limit;
  } while (newMembers.length === limit && offset <= 1000);
