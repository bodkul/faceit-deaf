...
function getLevelColor(level: number): string {
  ...
  if (level >= 2 && level <= 3) return LEVEL_COLORS[1];
  if (level >= 4 && level <= 7) return LEVEL_COLORS[2];
  if (level >= 8 && level <= 9) return LEVEL_COLORS[3];
  if (level === 10) return LEVEL_COLORS[4];
  return LEVEL_COLORS[0];
}

export default function SkillLevelIcon({
  level,
  ...props
}: SkillLevelIconProps): JSX.Element {
  const color = getLevelColor(level);

  const title = `Уровень мастерства ${level}`;
